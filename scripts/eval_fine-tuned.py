"""
Evaluates a downstream model on all datasets specified in the config file.
"""
...
def main(cfg: DictConfig):
    ...
    model = instantiate(cfg.model.downstream)
    model.eval()
    model.to(cfg.general.device)
    # evaluate on all datasets specified in the config
    for evaluator in cfg.training.downstream.trainer.evaluators.values():
        dataset = evaluator.dataset
        evaluator_inst = instantiate(evaluator)
        evaluator_inst.evaluate(model)
        mf1 = ResultTracker.get(dataset).get_last_macro_f1_score()
        logger.info(f'MF1 on dataset "{dataset}": {mf1:.4f}')
