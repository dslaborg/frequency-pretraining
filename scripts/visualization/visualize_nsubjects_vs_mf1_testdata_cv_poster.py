...
expid_to_label = {
    ...
    "exp001a_1": "Traditional supervised training (no pretraining)",
    # "exp001b_2": "Fixed feature extractor",
    "exp001b_2": "Pretrained feature extractor (no fine-tuning)",
    # "exp001c_1": "Fine-tuned feature extractor",
    "exp001c_1": "Pretrained feature extractor (with fine-tuning)",
...
def main():
    ...
    bar_width = 1 / (len(exps_sorted) + 1)
    color_idcs = np.linspace(0.2, 0.8, len(exps_sorted))
    for j, exp in enumerate(exps_sorted):
        color = my_cmap(color_idcs[j])
        ...
        rects = plt.bar(
            np.arange(len(means)) + j * bar_width,
            means,
            yerr=stds,
            width=bar_width,
            color=color,
    ...
    plt.grid()
    plt.xlabel("Number of subjects in training data")
    plt.ylabel("Macro F1 score")
    # plt.legend(ncols=2)
    plt.legend()

    n_subjects_all = list(list(f1_scores.values())[0].keys())[::-1]
    print(bar_width, len(f1_scores))
    plt.xticks(
        np.arange(len(n_subjects_all)) + (len(f1_scores) - 1) * bar_width / 2,
        [f"{n:g}" for n in n_subjects_all],
    )
    y_ax_min = min(
        [np.mean(v) - np.std(v) for exp in f1_scores for v in f1_scores[exp].values()]
    )
    y_ax_max = max(
        [np.mean(v) + np.std(v) for exp in f1_scores for v in f1_scores[exp].values()]
    )
    # plt.ylim([0.25, 0.86])
    plt.ylim([y_ax_min - 0.02, y_ax_max + 0.04])

    plt.title("Macro F1 scores for a varying number of training subjects")
